...
dev-front:
	...
	if [ -n "$$VIRTUAL_HOST_VALUE" ] && ! printf '%s' "$$VIRTUAL_HOST_VALUE" | grep -Eq '^(https?://)?(localhost|127\.0\.0\.1)(:[0-9]+)?/?$$'; then \
	  CLOUDPUB_PROFILE=",cloudpub"; \
	else \
	  CLOUDPUB_PROFILE=""; \
	fi; \
	COMPOSE_PROFILES=frontend$$CLOUDPUB_PROFILE $(DOCKER_COMPOSE) --env-file .env up --build
...
dev-php:
	...
	VIRTUAL_HOST_VALUE=$$(grep -E '^VIRTUAL_HOST=' .env 2>/dev/null | tail -n1 | cut -d= -f2- | tr -d "'\""); \
	if [ -n "$$VIRTUAL_HOST_VALUE" ] && ! printf '%s' "$$VIRTUAL_HOST_VALUE" | grep -Eq '^(https?://)?(localhost|127\.0\.0\.1)(:[0-9]+)?/?$$'; then CLOUDPUB_PROFILE=",cloudpub"; else CLOUDPUB_PROFILE=""; fi; \
	...
	if [ "$$ENABLE_RABBITMQ_VALUE" = "1" ]; then \
	  PROFILES="frontend,php,queue,$$DB_PROFILE$$CLOUDPUB_PROFILE"; \
	else \
	  PROFILES="frontend,php,$$DB_PROFILE$$CLOUDPUB_PROFILE"; \
	fi; \
...
dev-node:
	...
	if [ -z "$$ENABLE_RABBITMQ_VALUE" ]; then ENABLE_RABBITMQ_VALUE=0; fi; \
	if [ "$$ENABLE_RABBITMQ_VALUE" = "1" ]; then \
	  PROFILES="frontend,node,queue,$$DB_PROFILE$$CLOUDPUB_PROFILE"; \
	else \
...
prod-php:
	...
	COMPOSE_PROFILES=php BUILD_TARGET=production $(DOCKER_COMPOSE) up --build -d

prod-python:
	@echo "Starting prod python environment"
	COMPOSE_PROFILES=python BUILD_TARGET=production $(DOCKER_COMPOSE) up --build -d

prod-node:
	@echo "Starting prod node environment"
	COMPOSE_PROFILES=node BUILD_TARGET=production $(DOCKER_COMPOSE) up --build -d
