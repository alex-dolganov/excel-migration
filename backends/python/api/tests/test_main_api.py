...
class GetTokenApiTest(TestCase):
    ...
    def test_get_token_does_not_trust_portal_admin_flag_from_request(self, oauth_from_dict, update_or_create):
        ...
        self.assertIs(account.is_b24_user_admin, False)
        account.save.assert_not_called()

    @patch("main.utils.decorators.auth_required.Bitrix24Account.update_or_create_from_oauth_placement_data")
    @patch("main.utils.decorators.auth_required.OAuthPlacementData.from_dict")
    def test_get_token_refreshes_portal_admin_flag_from_trusted_user_admin_api(self, oauth_from_dict, update_or_create):
        oauth_from_dict.return_value = SimpleNamespace()

        account = MagicMock()
        account.b24_user_id = 7
        account.is_b24_user_admin = False
        account.client.user.admin.return_value = SimpleNamespace(result=[{"ID": "7"}])
        account.create_jwt_token.return_value = "jwt-token"
        update_or_create.return_value = (account, False)
        ...
        account.client.user.admin.assert_called_once_with()
        account.save.assert_called_once_with(update_fields=["is_b24_user_admin"])

    @patch("main.utils.decorators.auth_required.Bitrix24Account.update_or_create_from_oauth_placement_data")
    @patch("main.utils.decorators.auth_required.OAuthPlacementData.from_dict")
    def test_get_token_clears_stale_portal_admin_flag_when_user_is_not_in_trusted_admin_list(self, oauth_from_dict, update_or_create):
        oauth_from_dict.return_value = SimpleNamespace()

        account = MagicMock()
        account.b24_user_id = 7
        account.is_b24_user_admin = True
        account.client.user.admin.return_value = SimpleNamespace(result=[{"ID": "15"}])
        account.create_jwt_token.return_value = "jwt-token"
        update_or_create.return_value = (account, False)
